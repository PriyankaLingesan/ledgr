...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 px-6 py-14 text-center">
      <div className="mb-1 flex h-8 w-8 items-center justify-center rounded-md border border-border bg-surface-2 text-ink-3">
...
export function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 px-6 py-12 text-center">
      <div className="flex h-8 w-8 items-center justify-center rounded-md border border-negative/30 bg-negative-soft text-negative">
        <AlertTriangle className="h-4 w-4" strokeWidth={1.8} />
      </div>
      <p className="text-[0.8125rem] font-medium text-ink">
        {api ? api.message : 'Something went wrong loading this view.'}
      </p>
      <div className="flex items-center gap-2 text-2xs text-ink-3">
        {api?.code && <span className="num rounded-xs border border-border bg-surface-2 px-1.5 py-px">{api.code}</span>}

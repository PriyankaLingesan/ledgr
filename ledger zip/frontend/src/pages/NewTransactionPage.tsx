...
export default function NewTransactionPage() {
  ...
  if (!description.trim()) problems.push('A description is required.');
  if (entries.length < 2) problems.push('A transaction needs at least two entries.');
  if (parsed.some((row) => !row.entry.accountId)) problems.push('Every entry needs an account.');
  if (parsed.some((row) => row.minor === null || row.minor <= 0))
    problems.push('Every entry needs a positive amount valid for the selected currency.');
  if (totalDebits === 0 || totalCredits === 0)
    problems.push('A transaction needs at least one debit and one credit.');
  if (totalDebits !== totalCredits) problems.push('Total debits must equal total credits.');

  const balanced =
    totalDebits === totalCredits && totalDebits > 0 && parsed.every((row) => row.minor !== null);
  const canSubmit = problems.length === 0 && !createTransaction.isPending;
  ...
  const submit = () => {
    createTransaction.mutate(
      ...
      {
        onSuccess: (transaction) => navigate(`/transactions/${transaction.id}`),
      },
    );
  };
    ...
    <>
      ...
      <div className="space-y-4 px-6 py-5">
        ...
        <Panel>
          <PanelHeader
            title="Entries"
            description="Direction carries the sign - amounts are always positive"
            actions={
          ...
          {eligible.length === 0 && !accountsQuery.isLoading && (
            <p className="flex items-center gap-2 border-b border-warning/30 bg-warning-soft px-4 py-2.5 text-xs text-ink-2">
              <AlertCircle className="h-3.5 w-3.5 text-warning" strokeWidth={1.9} />
                    ...
                    <div className="flex items-center gap-px rounded-sm border border-border p-0.5">
                      {(['DEBIT', 'CREDIT'] as const).map((direction) => (
                        <button
                          ...
                          className={cn(
                            'flex-1 rounded-xs px-2 py-1 text-2xs font-semibold uppercase tracking-wide transition-colors',
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Panel className="lg:col-span-2">
            <PanelHeader title="Pre-flight checks" description="The same rules the API enforces" />
            <ul className="divide-y divide-border">
              {problems.length === 0 ? (
                <li className="flex items-center gap-2 px-4 py-2.5 text-xs text-positive">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-positive" />
                  Ready to post — debits equal credits and every entry is valid.
                </li>
              ) : (
                problems.map((problem) => (
                  <li key={problem} className="flex items-center gap-2 px-4 py-2.5 text-xs text-ink-2">
            ...
            {apiError && (
              <div className="border-t border-border px-4 py-3">
                <p className="rounded-sm border border-negative/30 bg-negative-soft px-3 py-2 text-xs text-negative">
                  <span className="num font-semibold">{apiError.code}</span> — {apiError.message}
                </p>
                {Object.keys(apiError.details).length > 0 && (
                  <pre className="num mt-2 max-h-40 overflow-auto rounded-sm border border-border bg-surface-2 p-2 text-2xs text-ink-2">
          ...
          <Panel>
            <PanelHeader title="Idempotency" description="Sent as the Idempotency-Key header" />
            <div className="space-y-2 px-4 py-3">
              <div className="flex items-center gap-2">
                <code className="num flex-1 truncate rounded-sm border border-border bg-surface-2 px-2 py-1.5 text-xs text-ink-2">
                  {idempotencyKey}
                </code>
                <IconButton
              ...
              <p className="text-xs leading-relaxed text-ink-3">
                If this request times out and is retried with the same key, the server replays the
                original response instead of posting a second transaction.
              </p>
              <div className="flex items-center justify-between border-t border-border pt-2 text-xs">
                <span className="text-ink-3">Entries</span>
                <span className="num text-ink-2">{entries.length}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-ink-3">Net movement</span>
                <Amount
                  money={{ amount_minor: totalDebits, currency, amount: '' }}
                  showCurrency
                />
              </div>
            </div>

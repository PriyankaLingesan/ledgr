/**
 * Interface primitives, shared by the marketing site and the console.
 *
 * Small and square in the console - 5-7px radii, hairline borders, 28-32px
 * control heights, read-at-density. The marketing pages borrow the same
 * primitives at a larger size (`lg`) rather than inventing a second button
 * language.
 */
...
const BUTTON_SIZES: Record<ButtonSize, string> = {
  ...
  md: 'h-9 px-3.5 text-[0.8125rem] gap-2 rounded-sm',
  lg: 'h-11 px-5 text-[0.9375rem] gap-2.5 rounded-md',
};
...
export function Field({
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      <label htmlFor={htmlFor} className="field-label flex items-center gap-1">
        {label}
        {required && <span className="text-negative">*</span>}
      </label>
      {children}
      {error ? (
        <p className="text-[0.8125rem] text-negative">{error}</p>
      ) : hint ? (
...
export function PanelHeader({
  ...
  return (
    <header
      className={cn(
        'flex items-center justify-between gap-4 border-b border-border px-4 py-3.5',
        className,
      )}
    >
      <div className="min-w-0">
        <h2 className="truncate text-base font-semibold text-ink">{title}</h2>
        {description && <p className="mt-0.5 truncate text-[0.8125rem] text-ink-2">{description}</p>}
      </div>

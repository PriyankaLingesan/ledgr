...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 px-4 py-2.5">
      <dt className="shrink-0 text-[0.8125rem] text-ink-3">{label}</dt>
      <dd className="min-w-0 text-right text-[0.8125rem] text-ink-2">{children}</dd>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col">
      ...
      <ul className="flex-1 divide-y divide-border">
        {entries.length === 0 ? (
          <li className="px-4 py-6 text-center text-[0.8125rem] text-ink-3">
          ...
          entries.map((entry) => (
            <li key={entry.id} className="flex items-start justify-between gap-4 px-4 py-2.5">
              <div className="min-w-0">
                ...
                {entry.memo && <p className="mt-0.5 text-[0.8125rem] text-ink-3">{entry.memo}</p>}
              </div>
  ...
  return (
    <>
      <PageHeader
        breadcrumb={
          <>
            <Link to="/app/transactions" className="hover:text-ink">
              Transactions
            </Link>
              ...
              >
                ...
                {transaction.reverses_transaction_id ? (
                  <span className="text-ink-2">
                    ...
                    <Link
                      to={`/app/transactions/${transaction.reverses_transaction_id}`}
                  ...
                  <span className="text-ink-2">
                    ...
                    <Link
                      to={`/app/transactions/${transaction.reversed_by_transaction_id}`}
                      className="num font-medium text-accent hover:underline"
                    >
                  ...
                  <tfoot>
                    <tr className="bg-surface-2">
                      <td className="td" colSpan={5}>
                        <span className="caption font-medium text-ink">Totals</span>
                      </td>
                      <td className="td text-right">

...
def post_transaction(db: Session, cmd: PostCommand) -> Transaction:
    ...
    # `_write_transaction` is inside the try, not just `db.commit()`: a UNIQUE
    # violation on `reference` raises the moment its INSERT is flushed - which
    # `_write_transaction` does itself, to assign `transaction.id`/`seq` - not
    # only when the session finally commits. Catching just the commit let a
    # duplicate reference escape as a raw, untranslated IntegrityError.
    try:
...
def reverse_transaction(
    ...
    *,
    ...
    # Same reasoning as `post_transaction`: `_write_transaction` flushes its
    # own INSERT to assign the reversal's id/seq, so a UNIQUE violation on
    # `reverses_transaction_id` (a double-reversal that slips past the row
    # lock above) or on `reference` can raise there, before `db.commit()` is
    # ever reached. Both must be inside the same try.
    try:
        reversal = _write_transaction(
            ...
            # A correction must always be possible. Refusing to reverse
            # because the result dips below a balance floor would trap the
            # ledger in a state it is not allowed to leave.
...
def load_transaction(db: Session, transaction_id: uuid.UUID) -> Transaction:
    transaction = db.execute(
        ...
        # Without this, a session that already loaded this row earlier in its
        # lifetime (e.g. `reverse_transaction` locking the original before
        # a reversal existed) would keep serving that stale relationship
        # state - `reversed_by` would read back None forever on this session
        # even after a reversal is committed. `populate_existing` forces this
        # read to refresh from the row that's actually in the database now,
        # same as `_load_balance_rows` does for the same reason.
        .execution_options(populate_existing=True)
    ).scalar_one_or_none()
